...
:::python
For building custom agents without these built-in capabilities, consider using LangChain's [`create_agent`](/oss/langchain/agents) or building a custom [LangGraph](/oss/langgraph/overview) workflow.
:::

:::js
For building custom agents without these built-in capabilities, consider using LangChain's [`createAgent`](/oss/langchain/agents) or building a custom [LangGraph](/oss/langgraph/overview) workflow.
:::
...
Pass custom functions, LangChain tools, or tools from any [MCP server](/oss/deepagents/tools#mcp-tools) with the `tools=` parameter. Deep Agents fully support the [Model Context Protocol (MCP)](/oss/langchain/mcp), letting you connect to databases, APIs, file systems, and more through a standard interface.

:::python
<OverviewToolsPy />
:::

For more information on defining custom tools, using MCP servers, and the full list of built-in harness tools, see [Tools](/oss/deepagents/tools).

### Virtual filesystem access

The harness provides a configurable virtual filesystem which can be backed by different [pluggable backends](/oss/deepagents/backends): in-memory state, local disk, LangGraph store, composite routing, or a custom backend with [permission rules](/oss/deepagents/permissions) for read and write access.
...
For configuration details, see [Context engineering](/oss/deepagents/context-engineering). For multimodal inputs and tool outputs, see [Multimodal](/oss/deepagents/multimodal).
...
For other providers, see [Middleware integrations](/oss/integrations/middleware#official-integrations) for available provider-specific caching middleware.
...
- **Configurable strategy**: Use the [default `general-purpose` subagent](/oss/deepagents/subagents#default-subagent) (enabled by default) or define [custom subagents](/oss/deepagents/subagents#custom-subagents).
...
<Accordion title="Running without subagents (no `task` tool)" icon="ban">
    To run an agent without the `task` tool, see [Running without subagents](/oss/deepagents/subagents#running-without-subagents). Do not try removing @[`SubAgentMiddleware`] via `excluded_middleware`—that is intentionally rejected. Instead, disable the auto-added subagent via the [harness profile](/oss/deepagents/profiles#harness-profiles) and pass no synchronous subagents via `subagents=`. Async subagents are unaffected. See the [default middleware stack](/oss/deepagents/customization#default-stack-main-agent) for the full ordering.
</Accordion>
